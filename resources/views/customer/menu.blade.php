...
@section('content')
    <div class="container-fluid page-header py-5">
        ...
        <ol class="breadcrumb justify-content-center mb-0">
            <li class="breadcrumb-item active text-primary">Berbagai pilihan mennu terbaik</li>
        </ol>
    </div>
    <!-- Fruits Shop Start-->
                                ...
                                @foreach ($items as $item)
                                    <div class="col-md-6 col-lg-6 col-xl-4">
                                        <div class="rounded position-relative fruite-item">
                                            ...
                                            <div class="text-white px-3 py-1 rounded position-absolute {{ $item->category->cat_name == 'Makanan' ? 'bg-warning' : ($item->category->cat_name == 'Minuman' ? 'bg-info' : 'bg-primary') }}"
                                                style="top: 10px; left: 10px;">
                                                {{ $item->category->cat_name }}
                                            </div>
                                            <div class="p-4 border border-secondary border-top-0 rounded-bottom">
                                                <h4>{{ $item->name }}</h4>
                                                <p class="text-limited">{{ $item->description }}
                                                </p>
